...
_TASK_CONFIG.SIMULATOR.DEPTH_SENSOR.WIDTH = 128
_TASK_CONFIG.SIMULATOR.DEPTH_SENSOR.HEIGHT = 128
_TASK_CONFIG.SIMULATOR.AGENT_0.SENSORS = ["RGB_SENSOR"]

# Online WayPixel costmap sensor (pirlnav/task/costmap_sensor.py).
# Intrinsics/depth params MUST match SIMULATOR.DEPTH_SENSOR (HFOV 79, 0.5-5.0m,
# normalized depth) -- the sensor backprojects observations["depth"].
_TASK_CONFIG.TASK.COSTMAP_SENSOR = CN()
_TASK_CONFIG.TASK.COSTMAP_SENSOR.TYPE = "CostmapSensor"
# 64x64: costmap is computed at this resolution (backprojection + KD lookup
# scale with pixel count; the geodesic field is low-frequency, and the policy
# resizes to its encoder size anyway). 128 -> 64 measured ~2-3x env-time win.
_TASK_CONFIG.TASK.COSTMAP_SENSOR.OUT_HEIGHT = 64
_TASK_CONFIG.TASK.COSTMAP_SENSOR.OUT_WIDTH = 64
_TASK_CONFIG.TASK.COSTMAP_SENSOR.HFOV = 79.0
_TASK_CONFIG.TASK.COSTMAP_SENSOR.SENSOR_HEIGHT = 0.88
_TASK_CONFIG.TASK.COSTMAP_SENSOR.MIN_DEPTH = 0.5
_TASK_CONFIG.TASK.COSTMAP_SENSOR.MAX_DEPTH = 5.0
_TASK_CONFIG.TASK.COSTMAP_SENSOR.NORMALIZE_DEPTH = True
_TASK_CONFIG.TASK.COSTMAP_SENSOR.GRID_RES = 0.2
_TASK_CONFIG.TASK.COSTMAP_SENSOR.MAX_COST = 30.0
...
# Dataset class used during eval. Default matches the standard val episodes;
# set to "ObjectNav-v2" to eval on HD-demo-format episodes (e.g. overfit tests).
_CONFIG.EVAL.DATASET_TYPE = "ObjectNav-v1"
...
_CONFIG.POLICY.COSTMAP_ENCODER = CN()
_CONFIG.POLICY.COSTMAP_ENCODER.image_size = 256
_CONFIG.POLICY.COSTMAP_ENCODER.backbone = "resnet50"
_CONFIG.POLICY.COSTMAP_ENCODER.resnet_baseplanes = 32
_CONFIG.POLICY.COSTMAP_ENCODER.hidden_size = 512
_CONFIG.POLICY.COSTMAP_ENCODER.avgpooled_image = False
_CONFIG.POLICY.COSTMAP_ENCODER.drop_path_rate = 0.0
_CONFIG.POLICY.COSTMAP_ENCODER.normalize_visual_inputs = True
_CONFIG.POLICY.COSTMAP_ENCODER.max_cost = 30.0
