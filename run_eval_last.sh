...
python -u -m run \
  ...
  --run-type eval \
  TENSORBOARD_DIR tb/il_repro_eval_last \
  EVAL_CKPT_PATH_DIR data/new_checkpoints/il_repro/ckpt.9.pth \
  NUM_ENVIRONMENTS 4 \
  TEST_EPISODE_COUNT -1 \
  WRITER_TYPE wb \
  WB.PROJECT_NAME pirlnav-baseline \
  WB.RUN_NAME il_ovrl_repro20k_eval_last \
